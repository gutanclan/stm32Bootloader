...
#define STR_QUEUE_CIR_INVALID_CHAR_SUBSTITUTE               ('?')


// declare number of elements per queue
#define STR_QUEUE_CIR_CONSOLE_1_BUFFER_SIZE_BYTES           (100)
...
{
    //	*pcBuffer						MAX SIZE
    {   &gcQueue1Buffer[0],             sizeof(gcQueue1Buffer)   },
    // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
};
        ...
        if( eStrQueueCir < STR_QUEUE_CIR_ARRAY_MAX )
        {
            ...
            if( dwStringLen > 0 )
            {
                ...
                for( UINT32 dwCharCounter = 0 ; dwCharCounter < dwStringLen ; dwCharCounter++ )
                {
                    ...
                    {
                        // substitute it
                        cChar = STR_QUEUE_CIR_INVALID_CHAR_SUBSTITUTE;
                    }
                ...
                // on new insert browser index also gets updated to the start of next string
...
BOOL StrQueueCirGetStringAtCurrentIndex( StrQueueCirEnum eStrQueueCir, CHAR *pcStringBuffer, UINT32 dwStringBufferSize )
{
	...
	)
    {
		...
		if( gtQueueLookupArray[eStrQueueCir].bStringCounter != 0 )
        {
            ...
            // if string copied greater than 0 char
            if( dwIndex > 0 )
            {
                fSuccess = TRUE;
            }
...
{
	BOOL    fSuccess = FALSE;
	UINT32  dwStringBrowserIndexTemp;

	if( eStrQueueCir < STR_QUEUE_CIR_ARRAY_MAX )
    {
        if( gtQueueLookupArray[eStrQueueCir].bStringCounter != 0 )
        {
            dwStringBrowserIndexTemp = gtQueueLookupArray[eStrQueueCir].dwStringBrowserBufferIndex;

            dwStringBrowserIndexTemp = StrQueueCirBufferGetPreviousIndex( eStrQueueCir, dwStringBrowserIndexTemp );
            if( (gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] == STR_QUEUE_CIR_STRING_DELIMITER_END ) )
            {
                // find the next start, but if first find end, then do nothing
                do
                {
                    dwStringBrowserIndexTemp = StrQueueCirBufferGetPreviousIndex( eStrQueueCir, dwStringBrowserIndexTemp );

                    if( gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] == STR_QUEUE_CIR_STRING_DELIMITER_START )
                    {
                        // update
                        gtQueueLookupArray[eStrQueueCir].dwStringBrowserBufferIndex = dwStringBrowserIndexTemp;
                        fSuccess = TRUE;
                    }
                    else if( gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] == STR_QUEUE_CIR_STRING_DELIMITER_END )
                    {
                        // don't update
                        break;
                    }
                ...
                (
                    ( gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] != STR_QUEUE_CIR_STRING_DELIMITER_START ) &&
                    ( gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] != STR_QUEUE_CIR_STRING_DELIMITER_END )
                );
            }
        }
	}
	...
	if( eStrQueueCir < STR_QUEUE_CIR_ARRAY_MAX )
    {
        if( gtQueueLookupArray[eStrQueueCir].bStringCounter != 0 )
        {
            ...
            if( gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] == STR_QUEUE_CIR_STRING_DELIMITER_START )
            {
                do
                {
                    dwStringBrowserIndexTemp = StrQueueCirBufferGetNextIndex( eStrQueueCir, dwStringBrowserIndexTemp );
                }while( gtQueueLookupArray[eStrQueueCir].pcBuffer[dwStringBrowserIndexTemp] != STR_QUEUE_CIR_STRING_DELIMITER_END );
...
void StrQueueCirTest( void )
{
	...
	CHAR cTestBuffer2[3];
	memset( &cTestBuffer2[0], 0, sizeof(cTestBuffer2) );
	// buffer smaller than string to be retrieved
	StrQueueCirGetStringAtCurrentIndex( STR_QUEUE_CIR_CONSOLE_1, &cTestBuffer2[0], sizeof(cTestBuffer2) );
