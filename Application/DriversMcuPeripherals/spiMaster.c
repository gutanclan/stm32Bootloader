...
/*    presc Baud    */  SPI_BaudRatePrescaler_2,
			...
			if( SpiIsBusEnabled( eSpiBus ) == FALSE )
			{
				...
				for( UINT8 bSlave = 0; bSlave < SPI_SLAVE_COUNT ; bSlave )
				{
					...
					if( gtSpiSlaveLookupTable[bSlave].tSpiBus.eBusId == eSpiBus )
					{
						...
						if( GpioPortClockIsEnabled( gtSpiSlaveLookupTable[eSpiBus].tGpioCs.tGpioPort ) == FALSE )
						{
							GpioPortClockEnable( gtSpiSlaveLookupTable[eSpiBus].tGpioCs.tGpioPort, fEnable );
						}
						// prepare pin for output
						GpioPuPdSetVal( gtSpiSlaveLookupTable[eSpiBus].tGpioCs.tGpioPort, gtSpiSlaveLookupTable[eSpiBus].tGpioCs.bGpioPin, GPIO_PUPD_NOPUPD );
						GpioSpeedSetVal( gtSpiSlaveLookupTable[eSpiBus].tGpioCs.tGpioPort, gtSpiSlaveLookupTable[eSpiBus].tGpioCs.bGpioPin, GPIO_OSPEED_25MHZ );
						GpioOTypeSetVal( gtSpiSlaveLookupTable[eSpiBus].tGpioCs.tGpioPort, gtSpiSlaveLookupTable[eSpiBus].tGpioCs.bGpioPin, GPIO_OTYPE_PUSH_PULL );
						GpioModeSetOutput( gtSpiSlaveLookupTable[eSpiBus].tGpioCs.tGpioPort, gtSpiSlaveLookupTable[eSpiBus].tGpioCs.bGpioPin );
    ...
    if( eSpiBus < SPI_BUS_COUNT )
    {
        if( eSpiSlave < SPI_SLAVE_COUNT )
		{
			if( SpiIsBusEnabled( eSpiBus ) )
			{
				...
				// When sending or receiving bytes, pointer to gpios is checked to determine if MOSI or MISO can be used.
...
{
    BOOL        	fSuccess    = FALSE;
    SpiBusEnum  	eSpiBus     = SPI_BUS_UNKNOWN;
	SpiSlaveEnum 	eCurrentSlaveInBus;
	UINT8 			bByteToSend;
    UINT8 			bByteReceived;
    TIMER       	tWaitTimer  = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////
//!
//! \fn         BOOL SpiSendRecvByteArray( SpiSlaveEnum eSpiSlave, UINT8 *pbByteArraySend, UINT8 *pbByteArrayRecv, UINT16 wInOutByteArraySize )
//!
//! \brief      Send and/or receive a byte array to/from the spi bus
//!
//! \param[in]  eSpiSlave       Slave that is currently using the bus. Only this slave can use the bus
//!                             If any other slave that is in the same bus tries to use it,
//!                             it will fail because only one salve can use the bus.
//!
//! \param[in]  pbByteArraySend         byte array to send through the spi bus
//!
//! \param[in]  pbByteArrayRecv         byte array received from the spi bus
//!
//! \param[in]  wInOutByteArraySize     array size. Both send and receive arrays must be the same size.
//!
//! \return BOOLEAN
...
BOOL SpiSlaveSendRecvByteArray( SpiSlaveEnum eSpiSlave, const UINT8 * const pbByteArraySend, UINT8 * const pbByteArrayRecv, UINT16 wInOutByteArraySize )
{
    ...
    if( SpiIsBusReadyForSlaveUse( eSpiSlave, &eSpiBus ) )
    {
        // Check MOSI and MISO pins are not null
        if( ( gtSpiBusLookupTable[ eSpiBus ].tGpioMosi.ptPort == NULL ) ||
            ( gtSpiBusLookupTable[ eSpiBus ].tGpioMiso.ptPort == NULL ) )
        {
            return TRUE;    // Don't actually do transfer if dummy pins used
        }
