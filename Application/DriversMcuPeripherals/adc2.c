...
BOOL AdcEnableAdcChannel( AdcInputEnum eAdc, BOOL fEnable )
{
    ...
    if( eAdc < ADC_ARRAY_MAX )
    {
        if( fEnable )
        {
			...
			if
			(
				( gtAdcLookupArray[ePort].tPin.tGpioPort < GPIO_PORT_INVALID ) ||
				( gtAdcLookupArray[ePort].tPin.bGpioPin < GPIO_PIN_INVALID )
			)
			{
				if( GpioPortClockIsEnabled( gtAdcLookupArray[ePort].tPin.tGpioPort ) == FALSE )
				{
					GpioPortClockEnable( gtAdcLookupArray[ePort].tPin.tGpioPort, fEnable );
				}
				// prepare pin for ADC mode
				GpioSpeedSetVal( gtAdcLookupArray[ePort].tPin.tGpioPort, gtAdcLookupArray[ePort].tPin.bGpioPin, GPIO_OSPEED_25MHZ );
				GpioPuPdSetVal( gtSerialLookupArray[ePort].tPin.tGpioPort, gtSerialLookupArray[ePort].tPin.bGpioPin, GPIO_PUPD_NOPUPD );
				GpioModeSetAnalog( gtSerialLookupArray[ePort].tPin.tGpioPort, gtSerialLookupArray[ePort].tPin.bGpioPin );
			}
            //////////////////////////////////////////////////

			
			// Enable ADC clock
			AdcClockEnable( eAdc, fEnable );
...
BOOL AdcGpioGetPortPin( AdcInputEnum eAdc, GpioPortEnum *pePort, UINT8 *pbPinNumber )
{
	...
	if( eAdc < ADC_ARRAY_MAX )
	{
		if( pePort != NULL )
		{
			(*pePort) = gtAdcLookupArray[ePort].tPin.tGpioPort;
			
			if( (*pePort) > GPIO_PORT_INVALID )
			{
				(*pePort) = GPIO_PORT_INVALID;
			}
		}
		
		if( pbPinNumber!= NULL )
		{
			(*pbPinNumber) = gtAdcLookupArray[ePort].tPin.bGpioPin;
			
			if( (*pbPinNumber) > GPIO_PIN_INVALID )
			{
				(*pbPinNumber) = GPIO_PIN_INVALID;
			}
		}
...
BOOL AdcChannelIsEnabled( AdcInputEnum eAdc )
{
	...
	if( eAdc < ADC_ARRAY_MAX )
	{
		fIsEnabled = AdcClockIsEnabled( eAdc );
	}
...
//!
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL AdcClockEnable( AdcInputEnum eAdc, BOOL fEnable )
{
    BOOL fSuccess = FALSE;

    if( eAdc < ADC_ARRAY_MAX )
    {
        switch( gtAdcLookupArray[ePort].tAdcCommon.dwRcc )
        {
            case RCC_APB2Periph_ADC1:
            case RCC_APB2Periph_ADC2:
            case RCC_APB2Periph_ADC3:
                if( fEnable )
                {
                    RCC->APB2ENR |= gtAdcLookupArray[ePort].tAdcCommon.dwRcc;
                }
                else{
                    RCC->APB2ENR &= ~gtAdcLookupArray[ePort].tAdcCommon.dwRcc;
                }
...
//!
//////////////////////////////////////////////////////////////////////////////////////////////////
BOOL AdcClockIsEnabled( AdcInputEnum eAdc )
{
    BOOL fEnabled = FALSE;
    ...
    if( eAdc < ADC_ARRAY_MAX )
    {
        switch(gtAdcLookupArray[ePort].tAdcCommon.dwRcc)
        {
            ...
            case RCC_APB2Periph_ADC2:
            case RCC_APB2Periph_ADC3:
                fEnabled = ( ( RCC->APB2ENR & gtAdcLookupArray[ePort].tAdcCommon.dwRcc ) > 0 );
                break;
            default:
                break;
        }
